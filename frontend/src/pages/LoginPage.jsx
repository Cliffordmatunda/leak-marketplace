...
const LoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const username = usernameRef.current.value; // ✅ Changed
        ...
        try {
            await login(username, password); // ✅ Changed
            navigate('/dashboard');
        } catch (err) {
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="relative group">
                                ...
                                <input
                                    ...
                                    placeholder="••••••••"
                                />
                                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-2.5 text-gray-500 hover:text-white">
                                    {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                                </button>
                            </div>
                        </div>

                        {/* Mobile Captcha & Link (Keep existing mobile logic) */}
                        <div className="md:hidden block">
                            ...
                            <div className="flex gap-2">
                                ...
                                <input className="flex-1 px-3 bg-[#06070a] border border-gray-700 rounded text-white outline-none uppercase tracking-widest font-bold text-center" placeholder="CODE" maxLength={5} onChange={(e) => { if (captchaInputRef.current) captchaInputRef.current.value = e.target.value; }} />
                            </div>
                        </div>
                        ...
                        <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-lg shadow-lg flex items-center justify-center gap-2 mt-4">
                            {isLoading ? <RefreshCw className="animate-spin h-5 w-5" /> : (<>Enter Dashboard <ArrowRight className="h-4 w-4" /></>)}
                        </button>

                        <div className="md:hidden mt-6 text-center">
                            <p className="text-gray-400 text-sm">Don't have an account? <Link to="/signup" className="text-blue-500 hover:text-blue-400 font-bold hover:underline transition-all">Create Account</Link></p>
                        </div>
                    </form>
                    ...
                    <div className="flex flex-col justify-center md:border-l border-gray-800 md:pl-8 mt-8 md:mt-0">
                        <div className="bg-[#13151f] p-6 rounded-xl border border-gray-800 text-center">
                            ...
                            <div className="relative bg-[#06070a] h-20 rounded-lg border border-gray-700 mb-4 flex items-center justify-center cursor-pointer group select-none" onClick={generateCaptcha}>
                                <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/noise.png')]"></div>
                                <span className="text-3xl font-mono font-black text-gray-200 tracking-[0.5em] line-through decoration-wavy decoration-blue-600/50 italic transform -rotate-2">{captchaCode}</span>
